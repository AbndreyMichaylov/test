...
```
### 2. Получение списка опросов
Для получения опросов нужно пройти по следующему адресу:
```
...
### 3. Прохождение опроса
Для прохождения опроса нужно пройти по следующему адресу:
```
http://127.0.0.1:8000/api/v1/survey/start/
```
После чего в теле передать следующий json, где answer - это ответы(строка), а survey - опрос(идентификатор опроса полученный выше, целое число)
```json
{
  "answer" : "На море",
  "survey" : 41
