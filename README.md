...
1. Python3
2. virtualenv
3. Операционная система под управление Windows
...
```
Для прохождения опроса анонимно требуется передать следующий токен:
```
hKJ~kANmFBqC 70e7a07225da5488f362f8b07971cea22482d219
```
...
2. Так же под самый конец написания проекта, я понял, что нужно было бы это все делать под linux, т.к. там по другому создается виртуальное окружение и имеется curl, но перенос требовал бы еще времени, потому я тоже отказался от этого

Я готов выполнить любые правки или побочные задания если потребуется, в том числе устранить две вышеперечисленные проблемы.
---------------------------------
----------------------------------
#### Детали работы
